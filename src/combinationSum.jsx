...
function CombinationSumCalculator() {
  ...
  const calculateCombinations = () => {
    ...
    const findCombinations = (arr, remaining, index, path, res) => {
      ...
      if (path.filter(x => x === arr[index]).length < arr.filter(x => x === arr[index]).length) {
        path.push(arr[index]);
        findCombinations(arr, remaining - arr[index], index + 1, path, res);
        path.pop();
      }

      // Exclude the current number and continue recursively
  ...
  return (
    <div className="container">
      <h2 className='heading'>Combination Sum Calculator</h2>
      <div className="form-group">
        <label>Target Sum:</label>
        <div className="input-with-tooltip">
          <input type="number" value={target} placeholder='10' onChange={e => setTarget(e.target.value)} />
          <div className="item-hints">
            <div className="hint" data-position="4">
              <span className="hint-radius"></span>
              <span className="hint-dot"><GoInfo /></span>
              <div className="hint-content do--split-children">
                <p>Enter the target sum here.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="form-group">
        <label>Numbers:</label>
        <div className="input-with-tooltip">
          <input type="text" placeholder='5 2 3 6' value={numbers} onChange={e => setNumbers(e.target.value)} />
          <div className="item-hints">
            <div className="hint" data-position="4">
              <span className="hint-radius"></span>
              <span className="hint-dot"><GoInfo /></span>
              <div className="hint-content do--split-children">
                <p>Please input the numbers here, separated by spaces.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <button className="cta" onClick={calculateCombinations}>
  <span className="span">Calculate</span>
